...
class EN6033212(Document):
    def reset_submit(self):
        # this function will reset the submit status
        sql_query = """UPDATE `tabEN 60332 1 2` SET `docstatus` = 0 WHERE `name` = '{name}'""".format(name=self.name)
        ...
        new_comment.reference_doctype = "EN 60332 1 2"
